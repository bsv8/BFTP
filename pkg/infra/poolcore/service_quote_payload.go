...
func MarshalListenCycleQuotePayload(requestedDurationSeconds uint32, requestedUntilUnix int64) ([]byte, error) {
	return json.Marshal([]any{
		"bsv8-listen-cycle-offer-payload-v1",
...
func MarshalNodeReachabilityQueryQuotePayload(targetNodePubkeyHex string) ([]byte, error) {
	return json.Marshal([]any{
		...
		NormalizeClientIDLoose(targetNodePubkeyHex),
	})
}
...
func UnmarshalListenCycleQuotePayload(raw []byte) (ListenCycleServicePayload, error) {
	...
	if version != "bsv8-listen-cycle-offer-payload-v1" {
...
func UnmarshalNodeReachabilityQueryQuotePayload(raw []byte) (NodeReachabilityQueryServicePayload, error) {
	...
	out.TargetNodePubkeyHex = NormalizeClientIDLoose(out.TargetNodePubkeyHex)
	return out, nil
}
